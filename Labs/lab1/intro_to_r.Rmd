...
new chunk can be inserted by clicking on the *Insert Chunk* button.

Before you keep going type your name. Then click on *Knit HTML* and you'll see your document in a new pop-up window.
...
These packages have already been installed for you, however you will still
need to load them in your working environment. To do so, type the following 
in the console:
...
Note that these two lines of code also appear on top of your R Markdown
document. We need to load the packages both in the console and
in your R Markdown environment since these two environments work independently
of each other.

Going forward you will be asked to load any relevant packages at the beginning
of each lab, and the code for loading these packages will also be included
on top of your R Markdown document as well.
...
-   In what year did we see the most total number of births in the U.S.? *Hint:*
    ...
    in a descenting order with another function: `desc` (for descending order).
