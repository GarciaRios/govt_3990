...
```{r}
lns <- lns %>% mutate(natprob_r = haven::as_factor(natprob) %>% fct_infreq() %>% fct_rev())
```
...
lns <- lns %>% mutate(poord_r = 
                        fct_relevel(poordisc, "strongly disagree", "somewhat disagree", 
