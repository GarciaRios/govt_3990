...
<div id="boxedtext">
**A note on workspaces:** Before we get started with the lab, let's take a moment
...
download.file("https://raw.githubusercontent.com/GarciaRios/govt_3990/gh-pages/Labs/lab3/lab3.Rmd", destfile = "lab3.Rmd")
```
</div>
...
You should see in your Environment tab that two new objects appear: `kobe_basket` 
and `calc_streak`:

- `kobe_basket`: a data frame of 133 observations and 6 variables, where every
...
- `calc_streak`: a custom function that we will use to calculate streak lengths
of shots (more on this later).
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. Look at different games by subsetting your data with `filter`.
...
processes are independent if the outcome of one process doesn't effect the outcome of the second. If each shot that a player takes is an independent process, having made or missed your first shot will not affect the probability that you will make or miss your second shot.
...
-   Make sure your document includes tables and you reference the information in those table while providing your answers.

</div>

<div id="license">
Science should be open! Here at Cornell and everywhere
