...
population.  (if this doesn't work, it's beacuse they have not loaded `dplyr`)
...
- At this point read Exercice 1 and give them a couple of minuts to come out with some answers... ask the class what they did but you don't have to get the exact code just get broad ideas of what to do. But hopfully they estimated means if so.. ask around what they got, since they are using different seeds the random sample will be different and their  means will vary.
...
- Skip this excercise they whould be able to fiiguire it out
...
## Confidence intervals
 ...
 - Go over this section it's importat
...
- They already knkow this formula but show it again as a reminder
...
Let's finally calculate the confidence interval (try to explain what this line is doing or better yet ask them to expalin what the line will do beforer they run it)
...
Afetr running that line, for teaching purposes it might be usefull to also estimate the mean right there, Like this
...
samp %>%
  summarise(lower = mean(Lot.Area) - z_star_95 * (sd(Lot.Area) / sqrt(n)),
            ...
            upper = mean(Lot.Area) + z_star_95 * (sd(Lot.Area) / sqrt(n)))
```
 -  You can see that the estimated mean ovbiosily falls right in the middle.. .
...
- I don't know if you use `iflelse`... it's not very untuitive so hopefully they get it... This is the firt time they will use it.
...
third is the value we want if the logical statement yields a false result.
 ...
 - (In this case  we use `mutate` to create a varaible that would have two values: "yes" and "no". The values would be assingned usung `iflese`:  if the lower bound is less than  `mu` AND  the upper bund greater than `mu`, the assigned value would be "yes", OTHERWISE it will be "no")
