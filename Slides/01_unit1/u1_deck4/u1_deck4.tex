...
\begin{itemize}
\item Lab 1 Due today by midnight \pause - Questions? \pause

\item Problem set (PS) 1  Due  Feb 19 \pause
...
\item Calculate the difference in proportions of yawners \red{(treatment - control)}, and submit this value (value must be between 0 and 1) - \textbf{only one submission per team per simulation}
\item Repeat steps (1) - (4) 2 times
\end{enumerate}
}
...
\disc{Below is a randomization distribution of 100 simulated differences in means ($\bar{x}_c - \bar{x}_{nc}$). Calculate the proportion for the hypothesis test evaluating whether caffeine \underline{increases} average tapping rate.
\begin{center}
{\footnotesize
\begin{tabular}{l | c | c | c}
\hline
