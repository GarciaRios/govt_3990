...
\item Decks online \pause

\item Grades

\item Problem Set and Lab now due Friday
\end{itemize}
...
\item We are often interested in \hl{population parameters}. \pause

\item Since complete populations are difficult (or impossible) to collect data on, we use \hl{sample statistics} as \hl{point estimates} for the unknown population parameters of interest. \pause

\item Sample statistics vary from sample to sample. \pause

\item Quantifying how sample statistics vary provides a way to estimate the \hl{margin of error} associated with our point estimate. \pause
...
\begin{frame}[fragile]
\frametitle{Social Media Activity Survey}
Back in 2015 we surveyed all 146 students of GOVT 111 and asked them, among other things, about their social media activity. For instance, we asked how many social media accounts they had.  \pause
...
\item Now, lets, sample, with replacement, ten student IDs (the  white cell):
{\footnotesize
...
\item Calculate the sample mean of their answer: $(8+6+10+4+5+3+5+6+6+6) / 10 = 5.9$
...
\activity{Creating a sampling distribution}{Repeat this, now on your own, and report your sample mean.}
\begin{enumerate}
...
\item Calculate the sample mean, round it to 2 decimal places.
...
\begin{frame}[fragile]
\frametitle{Average number of Syracuse games attended}
